...
func TestFastSSZBeaconStateBellatrixRoundtrip(t *testing.T) {
	...
	// Create a fastssz BeaconStateBellatrix instance
	state := &spectests.BeaconStateBellatrix{}
	
	// Unmarshal the data using fastssz
	if err := state.UnmarshalSSZ(originalData); err != nil {
		t.Fatalf("Failed to unmarshal SSZ data with fastssz: %v", err)
	}
	
	// Log some basic info about the unmarshaled state
	t.Logf("Unmarshaled state (fastssz):")
	t.Logf("  Slot: %d", state.Slot)
	t.Logf("  Number of validators: %d", len(state.Validators))
	t.Logf("  Number of balances: %d", len(state.Balances))
	
	// Marshal the state back to SSZ using fastssz
	marshaledData, err := state.MarshalSSZ()
	if err != nil {
		t.Fatalf("Failed to marshal SSZ data with fastssz: %v", err)
	}
	...
	t.Run("UnmarshalMarshaled", func(t *testing.T) {
		state2 := &spectests.BeaconStateBellatrix{}
		if err := state2.UnmarshalSSZ(marshaledData); err != nil {
	...
	t.Run("HashConsistency", func(t *testing.T) {
		...
		hash1, err := state.HashTreeRoot()
		if err != nil {
			t.Fatalf("Failed to calculate hash of original state: %v", err)
		}
		
		// Unmarshal and hash again
		state3 := &spectests.BeaconStateBellatrix{}
		if err := state3.UnmarshalSSZ(marshaledData); err != nil {
			t.Fatalf("Failed to unmarshal for hash test: %v", err)
		}
		
		hash2, err := state3.HashTreeRoot()
		if err != nil {
	...
	t.Run("CompareWithFlexSSZ", func(t *testing.T) {
		// Calculate hash with fastssz
		fastHash, err := state.HashTreeRoot()
		if err != nil {
			t.Fatalf("Failed to calculate fastssz hash: %v", err)
		}
		...
		t.Logf("fastssz hash: 0x%x", fastHash)
		t.Logf("flexssz hash: 0x%x", ourHash)
		
		if fastHash == ourHash {
			t.Log("✓ Hashes match between fastssz and flexssz!")
		} else {
			t.Error("✗ Hash mismatch between implementations")
		}
	})
}